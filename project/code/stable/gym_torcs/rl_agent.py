...
def flatten_low_dim_observation(observation, raw_state):
    """Build a compact low-dimensional state vector for steer+accel+brake PPO."""
    features = [
        _as_array(observation.speedX),
        _as_array(observation.speedY),
        _as_array(observation.rpm / 10000.0),
        _as_array(observation.track),
        _as_array(observation.wheelSpinVel / 100.0),
...
class ActorCriticNet(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256, init_log_std=-1.2):
        super().__init__()
        ...
        if action_dim >= 2:
            with torch.no_grad():
                self.policy_head.bias[1] = 0.5

    def forward(self, state_tensor):
        actor_hidden = self.actor(state_tensor)
        critic_hidden = self.critic(state_tensor)
...
def load_compatible_state_dict(model, state_dict):
    ...
    compatible_items = list(state_dict.items())
    if any(key.startswith("backbone.") for key in state_dict):
        for key, source_tensor in state_dict.items():
            ...
            compatible_items.append((f"actor.{suffix}", source_tensor))
            compatible_items.append((f"critic.{suffix}", source_tensor))

    seen_keys = set()
    for key, source_tensor in compatible_items:
        if key.startswith("backbone."):
            suffix = key[len("backbone."):]
            if f"actor.{suffix}" in model_state or f"critic.{suffix}" in model_state:
                continue
        if key in seen_keys:
            continue
        seen_keys.add(key)
...
def load_checkpoint_payload(model, optimizer, payload):
    if isinstance(payload, dict) and "model_state_dict" in payload:
        ...
        optimizer_skipped_reason = None
        if optimizer is not None and payload.get("optimizer_state_dict"):
            # If weights were only partially transferred, the optimizer moments
            # almost certainly no longer match parameter shapes.
            if load_report["partial"] or load_report["skipped"]:
                optimizer_skipped_reason = "partial_model_transfer"
            else:
                try:
                    optimizer.load_state_dict(payload["optimizer_state_dict"])
                    optimizer_loaded = True
                except (ValueError, RuntimeError) as error:
                    optimizer_skipped_reason = f"optimizer_state_incompatible: {error}"
        metadata = {
            ...
            "optimizer_skipped_reason": optimizer_skipped_reason,
        }
    ...
    return load_report, {
        "optimizer_loaded": False,
        "optimizer_skipped_reason": "state_dict_only_checkpoint",
    }
...
class PPOAgent:
    ...
    def _distribution(self, state_tensor):
        ...
        log_std = torch.clamp(self.model.log_std, -3.0, -0.0)
